...
logistic_split <- initial_split(data_train, prop = 0.8, strata = strike_f)
data_logistic_train <- training(logistic_split)
data_logistic_train <- data_logistic_train %>%
  select(-geometry)
data_logistic_test <- testing(logistic_split)
data_logistic_test <- data_logistic_test %>%
  select(-geometry)
...
logistic_recipe <- 
  recipe(strike_f ~ median_inc + earners + travel_work + rent +
          assi_income + employed+poverty_rate+ba_rate+Authorized, data = data_logistic_train) |>
...
logistic_fit <- logistic_workflow %>% 
  fit(data = data_logistic_train)

# 5. Predict on the Test Data
test_predictions <- logistic_fit %>% 
  predict(new_data = data_test) %>% 
  bind_cols(data_logistic_test)
...
rf_data_train <- rf_data_train %>%
  select(-geometry)
rf_data_test <- testing(rf_split)
rf_data_test <- rf_data_test %>%
  select(-geometry)
